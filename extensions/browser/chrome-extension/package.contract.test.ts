...
describe("simplified Chrome extension package", () => {
  it("declares only relay, access, storage, watchdog, native bootstrap, and Side Panel permissions", () => {
  ...
  it("contains no copilot or page-share runtime", () => {
    const files = fs
    ...
    expect(files.join("\n")).not.toMatch(/copilot|page-share/iu);
  });
