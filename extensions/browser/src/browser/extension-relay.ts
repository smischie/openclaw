        ...
        if (parsed && typeof parsed === "object" && "method" in parsed) {
          ...
          if (method === "Target.attachedToTarget") {
            ...
            if (attached?.sessionId && attached?.targetInfo?.targetId) {
              ...
              // Evict stale session for the same targetId to prevent Playwright
              // "Duplicate target" assertion crash (e.g. after tab re-attach/restore).
              for (const [sid, target] of connectedTargets) {
                if (sid !== attached.sessionId && target.targetId === nextTargetId) {
                  connectedTargets.delete(sid);
                  broadcastToCdpClients({
                    method: "Target.detachedFromTarget",
                    params: { sessionId: sid, targetId: nextTargetId },
                    sessionId: sid,
                  });
                  break;
                }
